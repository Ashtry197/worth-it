...
**Files:**
- Create: `lib/types.ts`, `lib/ppp.ts`, `lib/__tests__/ppp.test.ts`
...
**Interfaces:**
- Consumes: `JobInput`, `ScoreResult`, `ScoreBreakdown`, `ScoreError` from `lib/types.ts`; `toPppUsd` from `lib/ppp.ts`; `resolveBenchmark` from `lib/benchmark.ts` (defined in Task 5 — write Task 5 first if executing strictly in order, or stub per Step 4 below).
- Produces: `calculateScore(input: JobInput): ScoreResult` and `STANDARD_HOURS_PER_YEAR` from `lib/scoring.ts`.
...
**Interfaces:**
- Consumes: `JobInput`, `EducationLevel`, `EmployerType` from `lib/types.ts`; `toPppUsd` from `lib/ppp.ts`; `medianAnnualWage`, `GLOBAL_MEDIAN_WAGE_USD` from `lib/medianWages.ts`; `STANDARD_HOURS_PER_YEAR` from `lib/scoring.ts`.
- Produces: `resolveBenchmark(input: JobInput): { hourly: number; source: "user" | "fallback" }`.

To avoid a circular import, `STANDARD_HOURS_PER_YEAR` is re-declared here rather than imported from `lib/scoring.ts`. A test asserts the two stay equal.
...
```typescript
import { describe, it, expect } from "vitest";
import { resolveBenchmark, BENCHMARK_STANDARD_HOURS } from "@/lib/benchmark";
import { STANDARD_HOURS_PER_YEAR } from "@/lib/scoring";
...
describe("resolveBenchmark", () => {
  it("keeps its standard-hours constant in step with the scoring engine", () => {
    expect(BENCHMARK_STANDARD_HOURS).toBe(STANDARD_HOURS_PER_YEAR);
  });
...
/** Must equal STANDARD_HOURS_PER_YEAR in lib/scoring.ts. Duplicated rather
 *  than imported to avoid a circular dependency; a test enforces equality. */
export const BENCHMARK_STANDARD_HOURS = 2080;
...
export function resolveBenchmark(
  ...
  if (input.expectedSalary && input.expectedSalary > 0) {
    return {
      hourly: toPppUsd(input.expectedSalary, input.country) / BENCHMARK_STANDARD_HOURS,
      source: "user",
    };
  }
  ...
  return { hourly: adjusted / BENCHMARK_STANDARD_HOURS, source: "fallback" };
}
```
