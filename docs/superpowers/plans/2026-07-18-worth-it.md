...
The dataset is 179 entries of World Bank PPP conversion factors, curated by the upstream MIT project. Extract it rather than retyping:
...
wc -l /tmp/ppp-table.txt   # expect 182 lines (declaration + 179 entries + closing + blank)
```
...
export const pppFactors: Record<string, number> = {
  // <-- paste the 179 extracted entries here, e.g.  'AF': 18.71,
};
...
- Produces: `averageAnnualWage: Record<string, number>`, `WAGE_SOURCE: string`, `WAGE_YEAR: number`, `GLOBAL_MEDIAN_WAGE_USD: number`, `GLOBAL_MEDIAN_SOURCE: string`, and `GLOBAL_MEDIAN_WAGE_YEAR: number` from `lib/medianWages.ts`.
 ...
 *
 * - `averageAnnualWage` is an OECD **mean**. OECD's AV_AN_WAGE publishes no
 *   median variant (its AGGREGATION_OPERATION dimension is MEAN-only), so a
 *   mean is what exists. Means run roughly 15-20% above medians because of
 *   the top tail, making this a harder bar than a median would be.
 * - `GLOBAL_MEDIAN_WAGE_USD` is an ILO **median**, and from a different year.
 *
 * Both are all-role country aggregates, so they are a coarse proxy for any
 * individual's market rate. The UI must label which one a given score used
 * and from which year — see `benchmarkSource` in ScoreBreakdown.
 */
export const WAGE_SOURCE = "OECD Average annual wages (AV_AN_WAGE), mean not median";
export const WAGE_YEAR = 0; // replace with the OECD reference year from Step 1
...
export const GLOBAL_MEDIAN_SOURCE = "ILO Global Wage Report"; // add the edition
export const GLOBAL_MEDIAN_WAGE_YEAR = 0; // replace with the ILO reference year
...
Run: `npm test -- scoring`
Expected: PASS, 22 tests. If `@/lib/benchmark` does not exist yet, complete Task 5 first.
...
**Interfaces:**
- Consumes: `JobInput`, `EducationLevel`, `EmployerType` from `lib/types.ts`; `toPppUsd` from `lib/ppp.ts`; `averageAnnualWage`, `GLOBAL_MEDIAN_WAGE_USD` from `lib/medianWages.ts`; `STANDARD_HOURS_PER_YEAR` from `lib/constants.ts`.
- Produces: `resolveBenchmark(input: JobInput): { hourly: number; source: "user" | "country-average" | "global-median" }`.

The three sources are different statistics from different years — a user
estimate, an OECD **mean** (2025, 12 countries), and an ILO **median** (2021,
everywhere else). `resolveBenchmark` must report which one it used so the UI can
label it; collapsing the two fallbacks into one `"fallback"` value would hide a
15-20% systematic difference in what the score is measured against.
...
The new imports these tests need, added to the top of the file:
